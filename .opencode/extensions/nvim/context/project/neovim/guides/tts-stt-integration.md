...
| File | Purpose |
|------|---------|
| `.opencode/hooks/tts-notify.sh` | Claude Code TTS hook (lifecycle + interactive) |
| `.opencode/hooks/wezterm-notify.sh` | WezTerm tab color notification hook |
...
1. Edit `.opencode/settings.json`, remove TTS hook entries from Notification hook
