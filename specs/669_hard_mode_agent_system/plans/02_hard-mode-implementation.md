...
The following decisions are surfaced for the user to confirm or override before or during implementation:

1. **State persistence (`effort_mode` in state.json)**: Should `--hard` be sticky per-task? If a task is first run with `--hard`, should subsequent commands auto-inherit hard mode? The research recommends adding an `effort_mode` field. This plan defers sticky mode to a follow-up task to keep scope manageable -- users must pass `--hard` each time for now.

2. **Orchestrate hard-mode scope**: `skill-orchestrate-hard` is the largest new file (~600-800 lines). It encodes H1 (per-phase dispatch), H5 (divergence audit trigger), H6 (convergence policing), and H4 (mandatory adversarial verification gate). This plan creates it as a full structural variant because these are loop-level changes, not prompt injection. The alternative (branching inside skill-orchestrate) would add complexity to an already 1,145-line file.

3. **`routing_hard` manifest key**: This plan creates the infrastructure for extensions to declare hard-mode routing via a `routing_hard` sibling key in manifests. No extension uses it in this task -- lean4/cslib hard variants are deferred to a follow-up.

4. **Token cost documentation**: Hard mode is estimated at 3-5x standard cost per task lifecycle. The plan includes documenting this in CLAUDE.md. Combined with --team (5x), --team --hard is approximately 15-25x -- this should be noted in the command output.
...
**Non-Goals**:
- Extension-specific hard variants (lean4, nix, neovim) -- deferred to follow-up tasks
- Sticky effort_mode in state.json -- deferred
- Adaptive auto-escalation (churn detection -> auto --hard suggestion) -- v2
- Contract lint or automated testing of hard-mode behavioral correctness -- deferred
...
This plan is fully sequential due to the layered dependency chain: routing infrastructure -> contracts -> agents -> skills -> team composability -> documentation.

---

### Phase 1: Routing Infrastructure and Command Refactoring [NOT STARTED]
...
**Tasks**:
- [ ] Extend `.claude/scripts/command-route-skill.sh` to accept a 4th argument `$4 = effort_flag` (default empty for backward compatibility). When `effort_flag="hard"`: (a) check `routing_hard.$operation.$task_type` in each extension manifest before standard routing; (b) if no extension hard variant found, construct candidate name by appending `-hard` to the resolved skill name (e.g., `skill-researcher` -> `skill-researcher-hard`); (c) check if `.claude/skills/${candidate}/SKILL.md` exists; (d) if it exists, use it; if not, fall back to the standard skill with a stderr note: `[route] No hard variant for $skill_name; using standard skill`
- [ ] Refactor `.claude/commands/research.md` STAGE 2: Replace the inline routing loop (lines 333-366) with: `source .claude/scripts/command-route-skill.sh "research" "$TASK_TYPE" "skill-researcher" "$EFFORT_FLAG"` followed by `skill_name="$SKILL_NAME"`. This removes ~35 lines of inline routing. Preserve the team-mode check above this block (team-mode takes precedence). Add a guard: if `TEAM_MODE="true"` AND `EFFORT_FLAG="hard"`, set a `hard_team_mode=true` variable and pass it through to the team skill invocation
- [ ] Refactor `.claude/commands/plan.md` STAGE 2: Same pattern as research.md -- replace inline routing loop (lines 337-370) with `source command-route-skill.sh "plan" "$TASK_TYPE" "skill-planner" "$EFFORT_FLAG"`. Preserve team-mode precedence. Add the `hard_team_mode` pass-through for --team --hard
- [ ] Update `.claude/commands/implement.md` STAGE 2 (line 125): Add `"$EFFORT_FLAG"` as 4th argument to the existing `command-route-skill.sh` call. Add the `hard_team_mode` pass-through for --team --hard
- [ ] Update `.claude/commands/orchestrate.md` STAGE 0: Source `parse-command-args.sh` (currently not sourced -- orchestrate does its own parsing). Extract `EFFORT_FLAG`. In STAGE 1b routing, when `EFFORT_FLAG="hard"`, append `-hard` to resolved agent names (e.g., `general-research-agent` -> `general-research-hard-agent`) and set a flag to route to `skill-orchestrate-hard` for the implementation loop
- [ ] Add `effort_flag` pass-through in all four commands' Skill tool invocation args strings (e.g., `effort_flag={EFFORT_FLAG}`)
...
### Phase 2: Prompt Contract Files [NOT STARTED]
...
### Phase 3: Hard-Mode Agent Variants [NOT STARTED]
...
**Goal**: Update all documentation to reflect the new hard-mode system: CLAUDE.md skill-to-agent mapping, command reference, routing tables, and a "when to use --hard" decision framework.
...
**Files to modify**:
- `.claude/CLAUDE.md` -- Add hard-mode skill/agent/command documentation (~60 lines added)
...
- [ ] **CLAUDE.md completeness**: Hard mode section exists with decision framework, cost documentation, composability notes
...
The refactoring of inline routing in research.md/plan.md to use command-route-skill.sh is independently valuable and could be kept even if the rest of hard mode is reverted.
